...
namespace ClassAttendance.WebUI.Services
{
    ...
    public class NativeAuthenticationService : INativeAuthenticationService
    {
        private readonly IApplicationUserRepository _repo;

        public NativeAuthenticationService(IApplicationUserRepository repo)
        {
            _repo = repo;
        }
        ...
        public async Task<AuthenticationResult> SignIn(HttpContext context, string email, string password)
        {
            ...
            if (user == null)
            {
                result.SetError("User not found, please register to get access");
